...
class DenseNet(nn.Module):
    ...
    def __init__(
        self,
        growth_rate: int = 32,
        block_config: Tuple[int, int, int] = (6, 12, 24, 16),
        ...
        self.features = nn.Sequential(OrderedDict([
            ('pool00', tdConv(nn.AvgPool2d(kernel_size=3, stride=2))),
            ('conv0', tdConv(nn.Conv2d(2, num_init_features, kernel_size=3, stride=1,
    ...
    def dropout_layer(self, x):
        ...
        d = torch.stack((d,) * (steps + additional_step), 2).to(x.device)
        return x * d


    def forward(self, x: Tensor, batch_idx = 1, epoch = 1) -> Tensor:
        ...
        out = self.fcspike1(out, self.lateralfc1(out.unsqueeze(1)).squeeze(1))
        if self.drop_rate > 0 and self.training:
            out = self.dropout_layer(out)
        out = self.classifier2(out)
        out = self.fcspike2(out, self.lateralfc2(out.unsqueeze(1)).squeeze(1))
        out = torch.sum(out, dim=2) / (steps + additional_step)
        if self.training:
...
def densenet123(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> DenseNet:
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_.
    """
    return _densenet('densenet123', 32, (6, 12, 24, 16), 64, pretrained, progress, **kwargs)
