...
def train(args, model, device, train_loader, optimizer, epoch, train_acc, train_ls):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss2 = loss1 + nn.MSELoss()(output2, target)
        ...
        correct += pred1.eq(target_label.view_as(pred1)).sum().item()
        correct += pred2.eq(target_label.view_as(pred2)).sum().item()
    ...
    if epoch > 1:
        if acc == torch.max(torch.Tensor(test_acc)):
            if (args.save_model):
                ...
                torch.save(state, "/nmnist/tmp/nmnist_highest.pt")
...
def main():
    ...
    checkpoint_path = '/nmnist/tmp/nmnist.pt'
    start_epoch = 1
    test_acc = []
    train_acc = []
    test_ls = []
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        modeltest = model
        test(args, modeltest, device, test_loader, epoch, test_acc, test_ls)
        ...
        if epoch % 5 == 0:
            if (args.save_model):
                ...
                torch.save(model, "/nmnist/tmp/nmnist.pt")
