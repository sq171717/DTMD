    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data / (steps + additional_step)), len(train_loader.dataset),
    ...
    if epoch > 1:
        if acc == torch.max(torch.Tensor(test_acc)):
            if (args.save_model):
                ...
                torch.save(state, "/dvscifar10/tmp/dvscifar10_highest.pt")
...
def main():
    ...
    checkpoint_path = '/dvscifar10/tmp/dvscifar10.pt'
    start_epoch = 1
    test_acc = []
    train_acc = []
    test_ls = []
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % 5 == 0:
            if (args.save_model):
                ...
                torch.save(state, "/dvscifar10/tmp/dvscifar10.pt")
